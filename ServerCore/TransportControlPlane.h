...
struct UsedRWind //Recv를 통해 사용한 Rwind의 버퍼 사이즈 값을 QoS에서 Logic Layer로 올리기전 상대방에게 처리된 패킷의 사이즈 값을 다시보내 RWind 회복에 도움
{
	uint8 bexsist = 0;
	int32 rwindsize = -1;
};
...
public:
	...
	TransportControlPlane(uint32 tickms = TICKMS);
