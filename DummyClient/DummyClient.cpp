...
#define PRVT_SERVERADDR L"192.168.219.147"
...
void PacketDeliverCondition(PlayerRef player)
{
	...
	cout << "플레이어 ID : " << player->client_Id << " 전송패킷 : " << GDeliveryManager->GetDispatchedPacketCount()
		<< " 성공패킷 : " << GDeliveryManager->GetDeliveredPacketCount()
				...
				while (true && sendNum >= 0)
				{
					...
					for (auto& p : GHostManager.GetPlayers())
					{
						if (p.second->client_Id != 0)
						{
							...
							auto sendBufferchatPkt2 = ServerPacketHandler::MakeReliableBuffer_Low(shortChatPkt);
